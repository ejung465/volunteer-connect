# THIS IS THE CORRECT AND FINAL DOCKERFILE CONTENT
...
# Use a robust Node.js image for the backend runtime (fix for native modules)
FROM node:22 
WORKDIR /app
...
# 2. Temporarily switch directory and install server dependencies (THE FINAL FIX)
WORKDIR /app/server
RUN npm ci --omit=dev

# 3. Switch back to the root application directory
WORKDIR /app
