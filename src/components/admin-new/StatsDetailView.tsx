...
interface StatsDetailViewProps {
    statId: string;
    items?: any[]; // Allow external data injection
}

export const StatsDetailView: React.FC<StatsDetailViewProps> = ({ statId, items }) => {
    ...
    const config = {
        members: {
            title: 'Total Members',
            value: items ? items.length.toString() : '1,248',
            icon: 'groups',
            ...
            history: [
                { label: 'Volunteers', value: items ? items.filter(i => i.role === 'volunteer').length.toString() : '850', sub: 'Active' },
                { label: 'Tutors', value: items ? items.filter(i => i.role === 'tutor').length.toString() : '398', sub: 'Active' }
            ],
            list: items || [
                {
                    name: 'Sarah Lee',
    ...
    const data = config[statId as keyof typeof config]; // Or fallback to config.members if not keyof

    if (!data) return <div>Stat not found</div>;

    // Filter list based on role if it exists
    const filteredList = (statId === 'members')
        ? data.list.filter((item: any) => filter === 'all' || (item.role && item.role.toLowerCase() === filter.toLowerCase()))
        : data.list;
            ...
            <div className="flex flex-col gap-6 pb-24 animate-in slide-in-from-right duration-300">
                ...
                {statId === 'members' && (
                    <div className="flex p-1 bg-slate-200 dark:bg-white/5 rounded-xl border border-slate-300 dark:border-white/10">
                        {['all', 'volunteer', 'student'].map((f) => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`flex-1 py-2 rounded-lg text-xs font-bold capitalize transition-all duration-300 ${filter === f
                    ...
                    <GlassCard className="w-full max-w-sm relative z-10 flex flex-col items-center p-6 animate-in zoom-in-95 duration-200 !bg-white/95 dark:!bg-slate-900/95 shadow-2xl">
                        ...
                        <div className="w-24 h-24 rounded-full mb-4 relative group">
                            ...
                            <div className="absolute bottom-1 right-1 bg-white dark:bg-slate-800 rounded-full p-1.5 border border-slate-100 dark:border-slate-700 shadow-sm">
                                <Icon name={selectedMember.role === 'student' ? 'school' : 'volunteer_activism'} className={`text-sm ${data.iconColor}`} filled />
                            </div>
                        </div>
